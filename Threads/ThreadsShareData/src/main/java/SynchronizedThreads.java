...
public class SynchronizedThreads {
    public static void main(String args[]){
        ...
        //put in some println statements here
    ...
    public void run() {
        ...
        try {
            ...
            for (int count = 0; count < 3; count++) {
                ...
                if (count<2) {
                    System.out.println("Before access: "+theData.getMyData()+" (From thread #: "+id+")");
                    theData.access(id, count);
                    System.out.println("After access: "+theData.getMyData()+" (From thread #: "+id+")");
                } else {
                    System.out.println("Before increment: "+theData.getMyData()+" (From thread #: "+id+")");
                    theData.increment(id,count);
                    System.out.println("After increment: "+theData.getMyData()+" (From thread #: "+id+")");
                }
            Thread.yield();
	        }
...
class ShareableData {
    ...
    synchronized public void access(int who, int count){
	    System.out.println("Shareable data with value " + myData + " accessed by thread " + who + " count is " + count);
    }
